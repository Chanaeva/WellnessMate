...
export default function PackagesPage() {
  ...
  const [showMembershipExistsAlert, setShowMembershipExistsAlert] =
    useState(false);
  ...
  const { data: punchCardTemplates, isLoading: isPunchCardsLoading } = useQuery<
    PunchCardTemplate[]
  >({
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow wellness-container py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-heading text-foreground mb-4">
            Sacred Paths of Wellness
          </h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Choose your path to wellness in the ancient tradition of Romulus and
            Remus, nurtured by the sacred waters.
          </p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-8 bg-primary/10 border border-primary/20">
            <TabsTrigger
              value="memberships"
              className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary hover:bg-primary/10 transition-colors duration-200"
          ...
          <TabsContent value="memberships" className="space-y-8">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Unlimited access to our thermal wellness sanctuary, blessed by
                ancient waters
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {membershipPlans?.map((plan, index) => {
                ...
                const theme =
                  planThemes[plan.planType as keyof typeof planThemes] ||
                  planThemes.basic;

                return (
                  <Card
                    key={plan.id}
                    className={`wellness-card relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:scale-105 ${theme.border} border-2 bg-gradient-to-br from-white to-neutral-50/50`}
                  >
                    <div
                      className={`h-40 bg-gradient-to-br ${theme.gradient} relative`}
                    >
                      <div className="absolute inset-0 bg-black/5"></div>
                      <div className="absolute inset-0 bg-[url('/api/placeholder/400/160')] bg-cover bg-center opacity-5"></div>
                      <div className="relative h-full flex flex-col items-center justify-center p-4">
                        <div
                          className={`${theme.iconBg} p-3 rounded-full mb-2`}
                        >
                          <Icon className={`h-8 w-8 ${theme.accentColor}`} />
                        </div>
                        <h3
                          className={`text-lg font-heading ${theme.accentColor} text-center font-semibold`}
                        >
                          {theme.title}
                        </h3>
                    ...
                    <CardHeader className="pb-4">
                      ...
                      <div
                        className={`text-3xl font-bold ${theme.accentColor}`}
                      >
                      ...
                      <div className="space-y-3">
                        {plan.features?.map((feature, index) => (
                          <div key={index} className="flex items-start gap-3">
                            <div
                              className={`${theme.iconBg} p-1 rounded-full mt-0.5`}
                            >
                              <Check
                                className={`h-3 w-3 ${theme.accentColor}`}
                              />
                            </div>
          ...
          <TabsContent value="day-passes" className="space-y-8">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Flexible visits to the thermal sanctuaries when your spirit
                calls
              </p>
            </div>
        ...
        <div className="mt-16 space-y-8">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              All plans include access to our premium thermal wellness
              facilities
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <Card className="wellness-card text-center">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Traditional dry heat therapy rooms maintained at optimal
                  temperatures for deep relaxation and detoxification.
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="wellness-card text-center">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Invigorating cold therapy pools designed to boost circulation
            ...
            <Card className="wellness-card text-center">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Luxury changing areas, premium towels, and all the amenities
                  you need for the perfect wellness session.
                </p>
              </CardContent>
            </Card>
          </div>
      ...
      <AlertDialog
        open={showMembershipExistsAlert}
        onOpenChange={setShowMembershipExistsAlert}
      >
        <AlertDialogContent className="max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base leading-relaxed">
              You already have an active membership. Only one membership is
              allowed at a time.
              <br />
              <br />
              To change your membership plan, please cancel your current
              membership first. Note that membership cancellations are effective
              immediately with no prorated refund for remaining time.
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog
        open={showAddedToCartDialog}
        onOpenChange={setShowAddedToCartDialog}
      >
        <AlertDialogContent className="max-w-md">
          <AlertDialogHeader>
            <div className="flex items-center gap-3 mb-2">
            ...
            <AlertDialogDescription className="text-base leading-relaxed pt-2">
                ...
                <div className="bg-muted/50 rounded-lg p-4 mb-4">
                  ...
                  <div className="text-primary font-bold text-xl mt-2">
                    {new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format(addedItem.price / 100)}
                    {addedItem.type === "membership" && (
              ...
              Would you like to proceed to checkout or continue browsing?
            </AlertDialogDescription>
          </AlertDialogHeader>

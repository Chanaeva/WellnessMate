...
- **Payment Integration**: Stripe for memberships and punch cards, including payment method management, history, webhook handling, and automatic tax collection via Stripe Tax. Stripe Terminal integration for physical card readers at kiosk - connection token endpoint is public for SDK init, all other Terminal endpoints require admin/staff auth. Kiosk membership purchases create recurring Stripe subscriptions (Terminal: PaymentIntent with setup_future_usage saves card, then subscription created with 30-day billing anchor; Online: subscription created directly with incomplete status). Day passes remain one-time PaymentIntents.

...
- **Membership Management**: Supports multiple plan types, status tracking (active, inactive, expired, frozen), auto-renewal, upgrade/downgrade, age verification (18+), and a one-active-membership-per-user policy. Packages and day passes support purchase channel configuration (kiosk, website, cart) allowing admins to control where each package can be purchased.

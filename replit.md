...
- **Unified Visit Logging**: The admin Check-ins tab now shows a single "Visit Log" combining member check-ins and guest waivers in one paginated, searchable table. Each entry shows a "Guest" (purple), "QR Code", or "Manual" badge. Stats cards show Today's Total (all visits), Member Check-ins, and Guest Visits separately. New backend endpoints: `GET /api/admin/unified-check-ins` (paginated, filterable by period and search) and `GET /api/admin/unified-check-ins/today-count` (returns `{members, guests, total}`).
- **Admin Configuration Tab**: A dedicated "Configuration" tab in the admin dashboard provides a centralized settings page with sections for Membership Settings (max memberships per purchase), Capacity & Sessions (daily capacity, waitlist notify threshold), Day Pass Settings, Notification Settings (admin alert email, from name, reminder lead time), and a raw "All Settings" view of every key in the `site_settings` table. Settings are saved inline with per-field Save buttons.
